...
onready var menu = get_node("CanvasLayer3/ParentSprite/Control")



func _process(delta):
	if menu.ClickedPlay == true:
		...
		var Hospital = get_node("Hospital")
		...
		var replayButton = get_node("CanvasLayer/GameOver/Button")
		var victoryBackTo = get_node("CanvasLayer/MarginContainer/VictoryScreen/Button")
...
func _on_Timer_timeout():
	if Civilians <= 15 and GameOver == false and WonGame == false and menu.ClickedPlay == true:
		Civilians += 1
...
func _on_PlayerInfectTimer_timeout():
	...
	if PlayerInfected == true and menu.ClickedPlay == true:
		Player.health -= 0.4


func _on_House1_body_entered(body):
	var Player = get_node("Player")
	var HouseWorld1 = get_node("Sprite4").position
	if body.name == "Player" or body.name == "PlayerCollider":
		Player.position = HouseWorld1
...
func _on_Area2D_body_entered(body):
	var Hospital = get_node("Hospital")
	var cupboard = get_node("Cupboard")
	
	Hospital.HasEntered == true
	Hospital.isInHospital = false
	
		
	if body.name == "Player" or body.name == "PlayerCollider" or body.name == "PlayerArena":
		body.position = Hospital.position
